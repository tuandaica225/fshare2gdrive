...
async function transfer(fshare_file, remote_drive, remote_path) {
	...
	try {
		...
		if (remote_drive === undefined || remote_path === undefined) {
			...
			transfer_cmd = `curl -s "${fshare_download_url}" | gclone rcat --stats-one-line -P --stats 2s ${rclone_path}`
...
async function genCmd(fshare_folder, remote_drive, remote_path, page=1, is_root_folder=true) {
	...
	try {
		...
		const promises = body.items.map(async item => {
			if (item.type === 1) {
				let cmd = `curl -s https://raw.githubusercontent.com/tuandaica225/fshare2gdrive/master/fshare2gdrive.js | tail -n+2 | node - "https://fshare.vn/file/${item.linkcode}" "${remote_drive}" "${remote_path.replace(/\/$/,'')}/${(is_root_folder ? body.current.name + '/' : '')}" | bash -s`
				console.log(cmd)
			}	else {
...
(async () => {
	try {
		if (args === undefined) {
			throw new Error('Invalid arguments!\nPlease input valid arguments. See https://github.com/tuandaica225/fshare2gdrive#usage for more details')
		}
	} catch (e) {
		console.error(RED, e)
		process.exit(1)
